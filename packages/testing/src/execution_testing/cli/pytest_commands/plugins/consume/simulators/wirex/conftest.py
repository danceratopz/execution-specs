...
per pre-allocation group, reused by every test in that group, with the
chain rewound to genesis between tests. What differs is how a test's
blocks reach the client. Instead of being handed over one
`engine_newPayload` call at a time, they are downloaded by the client
from a mock devp2p peer, so the blocks travel the client's production
full sync path.
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption(
        ...
        default=2,
        help=(
            "Skip fixtures with fewer blocks than this. A fixture's head "
            "block is delivered over the Engine API to name the sync "
            "target, so only the blocks before it travel over devp2p; at "
            "the default of 2 every executed test syncs at least one "
            "block from the peer."
        ),
    )
    group.addoption(
