...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    eth_config_group.addoption(
        ...
        help=(
            ...
            "If no config is provided then majority mode will be used for "
            "devnet testing (clients with different responses than the "
            "majority of clients will fail the test)."
        ),
    )
    eth_config_group.addoption(
        "--clients",
        ...
        help=(
            ...
            'Example: "besu,erigon,geth,nethermind,nimbusel,reth". '
            "If not provided, majority mode testing will be disabled."
        ),
    )
    eth_config_group.addoption(
        "--genesis-config-file",
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if genesis_config_file and genesis_config_url:
        pytest.exit(
            "Cannot specify both --genesis-config-file and "
            "--genesis-config-url."
        )

    if (genesis_config_file or genesis_config_url) and network_name:
        pytest.exit(
            "Cannot specify a network name when using "
            "--genesis-config-file or --genesis-config-url."
        )
    ...
    #   case 3: network_name
        ...
        if not network_configs_path.exists():
            pytest.exit(
                f'Networks file "{network_configs_path}" does not exist.'
            )
        try:
        ...
        if network_name not in network_configs.root:
            pytest.exit(
                f'Network "{network_name}" not found in '
    ...
    try:
        logger.debug(
            "Checking whether eth_config is supported by target rpc.."
        )
        eth_rpc.config()
        logger.debug(
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    ...
    if metafunc.definition.name == "test_eth_config_majority":
        ...
        endpoints = all_rpc_endpoints_dict.items()
        metafunc.parametrize(
            ["eth_rpc"],
            [
                pytest.param(
                ...
                for endpoint_name, rpc_endpoint in endpoints
            ],
            scope="function",
        )
