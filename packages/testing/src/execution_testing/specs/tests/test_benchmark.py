...
def test_split_transaction(
    ...
    assert len(split_txs) == expected_splits, (
        f"Expected {expected_splits} transactions "
        f"for {gas_benchmark_value_millions}M gas, got {len(split_txs)}"
    )
    ...
    assert total_gas == gas_benchmark_value, (
        f"Total gas {total_gas} doesn't match "
        f"benchmark value {gas_benchmark_value}"
    )
