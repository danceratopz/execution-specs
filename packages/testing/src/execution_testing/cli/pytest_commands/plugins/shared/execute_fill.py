...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        ...
        "that decays to it across an empty block; filling with "
        "--prepend-empty-block skips it because no genesis compensation "
        "exists that preserves the test's fee environment.",
    )
    config.addinivalue_line(
        "markers",
