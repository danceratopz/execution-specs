"""
Test_random_statetest307.

Ported from:
state_tests/stRandom/randomStatetest307Filler.json
"""

import pytest
from execution_testing import (
    Account,
    Address,
    Alloc,
    Bytes,
    Environment,
    StateTestFiller,
    Transaction,
    compute_create_address,
)
from execution_testing.vm import Op

REFERENCE_SPEC_GIT_PATH = "N/A"
REFERENCE_SPEC_VERSION = "N/A"


@pytest.mark.ported_from(
    ["state_tests/stRandom/randomStatetest307Filler.json"],
)
@pytest.mark.valid_from("Cancun")
@pytest.mark.pre_alloc_mutable
def test_random_statetest307(
    state_test: StateTestFiller,
    pre: Alloc,
) -> None:
    """Test_random_statetest307."""
    coinbase = Address(0xF759667DC49F533FFB827FA2B95DAC452E82F810)
    contract_0 = Address(0x095E7BAEA6A6C7C4C2DFEB977EFAC326AF552D87)
    sender = pre.fund_eoa(amount=0xDE0B6B3A7640000)

    env = Environment(
        fee_recipient=coinbase,
        number=1,
        timestamp=1000,
        prev_randao=0x20000,
        base_fee_per_gas=10,
        gas_limit=9223372036854775807,
    )

    # Source: raw
    # 0x6000355415600957005b60203560003555
    coinbase = pre.deploy_contract(  # noqa: F841
        code=Op.JUMPI(
            pc=0x9,
            condition=Op.ISZERO(Op.SLOAD(key=Op.CALLDATALOAD(offset=0x0))),
        )
        + Op.STOP
        + Op.JUMPDEST
        + Op.SSTORE(
            key=Op.CALLDATALOAD(offset=0x0), value=Op.CALLDATALOAD(offset=0x20)
        ),
        balance=46,
        nonce=0,
        address=Address(0xF759667DC49F533FFB827FA2B95DAC452E82F810),  # noqa: E501
    )
    # Source: raw
    #   # noqa: E501
    contract_0 = pre.deploy_contract(  # noqa: F841
        code=Op.PUSH32[0xF759667DC49F533FFB827FA2B95DAC452E82F810]
        + Op.PUSH32[0xC350]
        + Op.SLOAD(key=Op.PUSH32[0xF759667DC49F533FFB827FA2B95DAC452E82F810])
        + Op.PUSH32[0xC350]
        + Op.CALLDATACOPY(
            dest_offset=Op.PUSH32[0x0],
            offset=Op.PUSH32[0x0],
            size=Op.PUSH32[0xC350],
        )
        + Op.CREATE
        + Op.SSTORE,
        nonce=0,
    )

    tx = Transaction(
        sender=sender,
        to=contract_0,
        data=Bytes(
            ""  # noqa: E501
        ),
        gas_limit=100000,
        value=0x4ACA7F0D,
    )

    post = {
        contract_0: Account(storage={}, nonce=0),
        compute_create_address(
            address=compute_create_address(address=contract_0, nonce=0),
            nonce=0,
        ): Account.NONEXISTENT,
        compute_create_address(
            address=compute_create_address(address=contract_0, nonce=0),
            nonce=1,
        ): Account.NONEXISTENT,
        coinbase: Account(storage={}, nonce=0),
        compute_create_address(
            address=contract_0, nonce=0
        ): Account.NONEXISTENT,
    }

    state_test(env=env, pre=pre, post=post, tx=tx)
