...
The `hive/` directory contains `{hash}.json` files with generic genesis and `HIVE_*` environment variables, generated alongside `pre_alloc/` groups. These are consumed by benchmarkoor to dynamically generate client-native genesis files using hive's `mapper.jq` scripts.
