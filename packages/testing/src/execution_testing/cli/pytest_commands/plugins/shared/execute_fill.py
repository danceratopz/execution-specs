...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        ...
        "depend on absolute block numbers or block hashes; it fills without "
        "the prepended empty block because the extra block would shift "
        "every block position and silently change what the test verifies.",
    )
    config.addinivalue_line(
        "markers",
        ...
        "system work; it fills without the prepended empty block because "
        "the extra block would either fail on that setup or consume its "
