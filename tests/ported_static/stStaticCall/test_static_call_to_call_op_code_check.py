...
Ported from:
tests/static/state_tests/stStaticCall/static_callToCallOpCodeCheckFiller.json
"""
...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/stStaticCall/static_callToCallOpCodeCheckFiller.json",  # noqa: E501
    ],
)
@pytest.mark.valid_from("Byzantium")
@pytest.mark.pre_alloc_mutable
@pytest.mark.slow
def test_static_call_to_call_op_code_check(
    ...
    pre.deploy_contract(
        code=(
            ...
            + Op.JUMPI(
                pc=0x4B,
                condition=Op.EQ(
                    0x51A6B9D90FA023DBFE63296B65876F9CAF3FF3FD,
                    Op.CALLER,
                ),
            )
            + Op.SSTORE(key=0x1, value=0x2)
    ...
    pre.deploy_contract(
        ...
        balance=0xDE0B6B3A7640000,
        nonce=0,
        address=Address("0x51a6b9d90fa023dbfe63296b65876f9caf3ff3fd"),  # noqa: E501
    )
    # Source: LLL
    ...
    tx = Transaction(
        ...
        to=contract,
        data=bytes.fromhex(
            "00000000000000000000000051a6b9d90fa023dbfe63296b65876f9caf3ff3fd"
        ),
        gas_limit=1000000,
        value=100000,
    )
