...
class BaseTest(BaseModel):
    ...
    def check_exception_test(
        ...
        *,
        ...
        if negative_test_marker != exception:
            if exception:
                raise Exception(
                    "Test produced invalid block/transaction but was not "
                    "marked with the `exception_test` marker. Add the "
                    "`@pytest.mark.exception_test` decorator to the test."
                )
            else:
                raise Exception(
                    "Test didn't produce invalid block/transaction but was "
                    "marked with `exception_test` marker. Remove the "
    ...
    def update_pre_alloc_groups(
        ...
        if not hasattr(self, "pre"):
            raise AttributeError(
                ...
                "Pre-allocation groups only supported for test types "
    ...
    def compute_pre_alloc_group_hash(self) -> str:
        ...
        if not hasattr(self, "pre"):
            raise AttributeError(
                f"{self.__class__.__name__} does not have a 'pre' field. "
                "Pre-allocation group usage only supported for test types "
                "that define pre-allocs."
            )
