...
class RLPxSession:
    ...
    def _read_exactly(self, length: int) -> bytes:
        """Read exactly `length` bytes or raise `RLPxError`."""
        buffer = b""
        while len(buffer) < length:
            chunk = self._connection.recv(length - len(buffer))
            if not chunk:
    ...
    def read_message(self) -> Tuple[int, bytes]:
        ...
        if frame_size > MAX_FRAME_SIZE:
            raise RLPxError(f"frame of {frame_size} bytes is too large")
    ...
    def set_timeout(self, timeout: float) -> None:
        """Set the socket read timeout in seconds."""
...
def connect(
    ...
    def read_exactly(length: int) -> bytes:
        """Read exactly `length` bytes from the raw socket."""
        buffer = b""
        while len(buffer) < length:
            chunk = connection.recv(length - len(buffer))
            if not chunk:
                raise RLPxError("peer closed the connection mid-handshake")
            buffer += chunk
        return buffer

    size_prefix = read_exactly(2)
    ack = size_prefix + read_exactly(int.from_bytes(size_prefix, "big"))
