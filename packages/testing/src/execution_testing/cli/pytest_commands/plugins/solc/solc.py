...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if solc_bin:
        ...
        if not solc_bin:
            pytest.exit(
                "solc binary not found in PATH. Please install solc "
    ...
    if solc_version_semver < SOLC_EXPECTED_MIN_VERSION:
        pytest.exit(
            f"Unsupported solc version: {solc_version_semver}. "
            f"Minimum required version is {SOLC_EXPECTED_MIN_VERSION}",
            returncode=pytest.ExitCode.USAGE_ERROR,
        )
