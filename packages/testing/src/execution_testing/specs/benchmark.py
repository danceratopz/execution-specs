...
class BenchmarkCodeGenerator(ABC):
    ...
    def generate_repeated_code(
        ...
        *,
        ...
        # proportional to 'fixed_opcode_count' * 1000 across two contracts
        # (Loop M * Target N).

        # --- 1. Determine Inner Iterations (N) ---
        # Target Contract's loop count is determined by block filling, cap 1000
        #
        # 1a. Calculate 'max_iterations' to fill the block.
        # 1b. The Inner Iteration count (N) is capped at 1000.
        # 1c. If calculated N is less than 1000, use 500 as fallback count.

        # --- 2. Determine Outer Iterations (M) ---
        # Loop Contract's call count (M) ensures final total execution is
        # consistent.
        #
        # 2a. If N==1000: Set M=fixed_opcode_count. (Total: fixed*1000)
        # 2b. If N==500: Set M=fixed_opcode_count*2. (Total: fixed*2*500)
    ...
    def _validate_code_size(self, code: Bytecode, fork: Fork) -> None:
        ...
        if len(code) > fork.max_code_size():
            raise ValueError(
                f"Generated code size {len(code)} exceeds max allowed size "
...
class BenchmarkTest(BaseTest):
    ...
    supported_markers: ClassVar[Dict[str, str]] = {
        "blockchain_test_engine_only": "Only generate blockchain test engine",
    ...
    def model_post_init(self, __context: Any, /) -> None:
        ...
        if self.code_generator is not None:
            ...
            raise ValueError(
                "Cannot create BlockchainTest without code generator, "
    ...
    def _verify_target_opcode_count(
        ...
        # (currently only supported for evmone filling)
