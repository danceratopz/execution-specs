...
def pytest_configure(config: pytest.Config) -> None:  # noqa: D103
    ...
    if hive_simulator_url is None:
        pytest.exit(
            ...
            "and set the HIVE_SIMULATOR to the reported URL. "
            "For example, in bash:\n"
...
def pytest_addoption(parser: pytest.Parser) -> None:  # noqa: D103
    ...
    pytest_hive_group.addoption(
        ...
        help=(
            "The Hive simulator endpoint, e.g. http://127.0.0.1:3000. "
            "By default, the value is taken from the HIVE_SIMULATOR "
...
def pytest_report_header(
    ...
    if hive_info := get_hive_info(config.hive_simulator):  # type: ignore[attr-defined]
        ...
        for client in hive_info.client_file.root:
            client_json = client.model_dump_json(exclude_none=True)
            header_lines += [f"hive client ({client.client}): {client_json}"]
    return header_lines
...
def hive_test(
    ...
    try:
        ...
        pytest.exit(
            "Error: The 'test_case_description' fixture has not been "
            "defined by the simulator or pytest plugin using this plugin!"
        )
