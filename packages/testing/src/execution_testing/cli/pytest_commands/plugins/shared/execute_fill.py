...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if config.pluginmanager.has_plugin(
        ...
        for fixture_format in BaseFixture.formats.values():
            fmt = fixture_format.format_name.lower()
            config.addinivalue_line(
                "markers",
                f"{fmt}: {fixture_format.description}",
            )
        for (
            label,
        ...
        for execute_format in BaseExecute.formats.values():
            fmt = execute_format.format_name.lower()
            config.addinivalue_line(
                "markers",
                f"{fmt}: {execute_format.description}",
            )
        for (
            label,
    ...
    config.addinivalue_line(
        "markers",
        "compile_yul_with(fork): Always compile Yul source "
        "using the corresponding evm version.",
    )
    config.addinivalue_line(
        "markers",
    ...
    config.addinivalue_line(
        "markers",
        "exception_test: Negative tests that include "
        "an invalid block or transaction.",
    )
    config.addinivalue_line(
        "markers",
        "eip_checklist(item_id, eip=None): Mark a test as implementing "
        "a specific checklist item. The first positional parameter is the "
        "checklist item ID. The optional 'eip' keyword parameter specifies "
        "additional EIPs covered by the test.",
    )
    config.addinivalue_line(
        "markers",
        "derived_test: Mark a test as a derived test "
        "(E.g. a BlockchainTest that is derived from a StateTest).",
    )
    config.addinivalue_line(
        "markers",
    ...
    config.addinivalue_line(
        "markers",
        "mainnet: Specialty tests crafted for running on mainnet "
        "and sanity checking.",
    )
...
def test_case_description(request: pytest.FixtureRequest) -> str:
    ...
    description_unavailable = (
        "No description available - add a docstring to the python "
