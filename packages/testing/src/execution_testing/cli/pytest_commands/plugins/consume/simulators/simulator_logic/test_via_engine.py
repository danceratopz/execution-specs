...
def test_blockchain_via_engine(
    ...
    with timing_data.time("Get genesis block"):
        ...
        if genesis_block["hash"] != str(fixture.genesis.block_hash):
            ...
            logger.fail(
                f"Genesis block hash mismatch. Expected: {expected}, "
                f"Got: {got}"
            )
        ...
        for i, payload in enumerate(fixture.payloads):
            ...
            with total_payload_timing.time(
                ...
                with payload_timing.time(
                    ...
                    ver = payload.new_payload_version
                    logger.info(f"Sending engine_newPayloadV{ver}...")
                    try:
                        payload_response = engine_rpc.new_payload(
                            *payload.params,
                            version=payload.new_payload_version,
                        )
                        logger.info(f"Payload status: {payload_response.status}")
                        expected_validity = (
                        ...
                        if payload.error_code is not None:
                            raise LoggedError(
                                f"Client failed to raise expected Engine API "
                            ...
                            if payload_response.validation_error is None:
                                raise LoggedError(
                                    "Client returned INVALID but no validation "
                                    "error was provided."
                                )
                            if isinstance(
                                ...
                                message = (
                                    ...
                                    f'returned: "{payload_response.validation_error}" '
                                    f"(mapper: "
                                    f'"{payload_response.validation_error.mapper_name}")'
                                )
                                ...
                                if (
                                    ...
                                    message = (
                                        "Client returned unexpected validation "
                                        f'error: got "{payload_response.validation_error}" '
                                        f'expected "{payload.validation_error}"'
                                    )
                                    if strict_exception_matching:
                                        raise LoggedError(message)
