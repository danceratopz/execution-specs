    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason=(
                    "Extra data is excluded=True in the Environment model, "
                    "so it does not propagate correctly to the genesis header "
...
def test_pre_alloc_grouping_by_test_type(
    ...
    if (
        ...
        error_message = (
            f"Expected {expected_different_pre_alloc_groups} "
            f"different pre-alloc groups, but got {len(groups)}"
        )
