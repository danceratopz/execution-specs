...
def test_blockchain_via_sync(
                ...
                with payload_timing.time(
                    ...
                    try:
                        ...
                        if payload.error_code is not None:
                            raise LoggedError(
                                "Client failed to raise expected Engine "
                                f"API error code: {payload.error_code}"
                            )
                        elif (
                            ...
                            if isinstance(
                                ...
                                expected = payload.validation_error
                                returned = payload_response.validation_error
                                mapper = returned.mapper_name
                                message = (
                                    "Undefined exception message: "
                                    f'expected exception: "{expected}", '
                                    f'returned exception: "{returned}" '
                                    f'(mapper: "{mapper}")'
                                )
                                ...
                                if (
                                    ...
                                    got = payload_response.validation_error
                                    exp = payload.validation_error
                                    message = (
                                        "Client returned unexpected "
                                        f'validation error: got: "{got}" '
                                        f'expected: "{exp}"'
                                    )
                                    if strict_exception_matching:
                                        raise LoggedError(message)
                ...
                if payload.valid():
                    with payload_timing.time(
                        ...
                        if (
                            ...
                            got = forkchoice_response.payload_status.status
                            raise LoggedError(
                                "unexpected status: "
                                f"want {PayloadStatusEnum.VALID}, got {got}"
                            )
    ...
    if not fixture.sync_payload:
        pytest.fail(
            "Sync tests require a syncPayload "
            "that is not present in this test."
        )
    ...
    with timing_data.time("Send sync payload to client under test"):
        ...
        try:
            ...
            logger.error(
                "Error sending sync payload to client under test: "
                f"{e.code} - {e.message}"
            )
            raise
    ...
    with timing_data.time("Initialize sync client"):
        ...
        try:
            ...
            if response.payload_status.status != PayloadStatusEnum.VALID:
                raise LoggedError(
                    "Unexpected status on sync client forkchoice "
                    f"updated to genesis: {response.payload_status.status}"
                )
        except ForkchoiceUpdateTimeoutError as e:
            raise LoggedError(
                "Timed out waiting for sync client forkchoice "
                f"update to genesis: {e}"
            ) from None
    ...
    if last_valid_payload:
        ...
        try:
            ...
            logger.info(
                "Sending forkchoice update with last valid block "
                "to trigger sync..."
            )
            ...
            if (
                ...
                logger.info(
                    "Sync client accepted the block, "
                    "may start syncing ancestors"
                )

            # Wait for P2P connections after sync starts
            # Note: Reth does not report peer count but still syncs
            try:
                assert sync_net_rpc is not None, "sync_net_rpc is required"
                sync_net_rpc.wait_for_peer_connection()
                logger.debug(
                    "Peer connection verified on sync client "
                    "after sync trigger"
                )
            except PeerConnectionTimeoutError:
                try:
            ...
            logger.warning(
                "Failed to trigger sync with "
                f"newPayload/forkchoice update: {e}"
            )
    else:
        logger.warning(
            f"Could not find payload for block {last_valid_block_hash} "
            "to send to sync client"
        )
    ...
    with timing_data.time("Wait for synchronization"):
        ...
        try:
            ...
            if response.payload_status.status != PayloadStatusEnum.VALID:
                raise LoggedError(
                    "Sync client failed to sync to block "
                    f"{last_valid_block_hash}: unexpected status "
                    f"{response.payload_status.status}"
                )
        except ForkchoiceUpdateTimeoutError as e:
            raise LoggedError(
                "Sync client timed out syncing to block "
