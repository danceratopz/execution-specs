    ...
    def get_pre_alloc_group(self, hash_key: str) -> PreAllocGroup:
        ...
        if hash_key not in self.pre_alloc_groups:
            ...
            raise ValueError(
                ...
                f"pre-allocation groups. Please check the pre-allocation "
                f"groups file at: {pre_alloc_path}. Make sure phase 1 "
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    evm_group.addoption(
        ...
        default=None,
        help=(
            "[INTERNAL USE ONLY] URL of the t8n server to use. "
            "Used by framework tests/ci; not intended for regular CLI use."
        ),
    )
    evm_group.addoption(
        "--traces",
        ...
        default=None,
        help=(
            "Collect traces of the execution information from the "
            "transition tool."
        ),
    )
    evm_group.addoption(
    ...
    test_group.addoption(
        ...
        type=int,
        help=(
            "Default gas limit used ceiling used for blocks and tests "
            "that attempt to consume an entire block's gas. "
    ...
    test_group.addoption(
        ...
        help=(
            ...
            " This enables two-phase execution: Phase 1 generates "
            "pre-allocation groups, phase 2 generates all supported "
            "fixture formats."
        ),
    )
    ...
    optimize_gas_group.addoption(
        ...
        default=False,
        help=(
            "Attempt to optimize the gas used in every transaction for "
            "the filled tests, then print the minimum amount of gas at "
            "which the test still produces a correct post state and the "
            "exact same trace."
        ),
    )
    optimize_gas_group.addoption(
    ...
    optimize_gas_group.addoption(
        ...
        type=int,
        help=(
            "Maximum gas limit for gas optimization, if reached the "
            "search will stop and fail for that given test. "
            "Requires `--optimize-gas`."
        ),
    )
    optimize_gas_group.addoption(
        ...
        default=False,
        help=(
            "Post process the traces during gas optimization in order "
            "to account for opcodes that put the current gas in the "
            "stack, in order to remove remaining-gas from the comparison."
        ),
    )
    ...
    if "passed" in stats and stats["passed"]:
        ...
        if session_instance.phase_manager.is_pre_alloc_generation:
            ...
            terminalreporter.write_sep(
                "=",
                f" Phase 1 Complete: Generated {total_groups} "
                f"pre-allocation groups ({total_accounts} total accounts) ",
                bold=True,
                green=True,
            )
...
def t8n(
    ...
    if not t8n.exception_mapper.reliable:
        warnings.warn(
            f"The t8n tool that is currently being used to fill tests "
            f"({t8n.__class__.__name__}) does not provide reliable "
            "exception messages. This may lead to false positives when "
            "writing tests and extra care should be taken when writing "
            "tests that produce exceptions.",
            stacklevel=2,
        )
    yield t8n
    t8n.shutdown()
...
def evm_fixture_verification(
    ...
    try:
        ...
        if reused_evm_bin:
            pytest.exit(
                "The binary specified in --evm-bin could not be "
                "recognized as a known FixtureConsumerTool. Either "
                "remove --verify-fixtures or set --verify-fixtures-bin "
                "to a known fixture consumer binary.",
                returncode=pytest.ExitCode.USAGE_ERROR,
            )
        else:
            pytest.exit(
                ...
                "`GethFixtureConsumer` for an example of how a new "
                "fixture consumer can be defined.",
                returncode=pytest.ExitCode.USAGE_ERROR,
            )
