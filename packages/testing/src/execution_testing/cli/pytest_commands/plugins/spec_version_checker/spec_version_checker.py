...
from execution_testing.base_types.reference_spec import (
    ReferenceSpec,
    ReferenceSpecTypes,
)
...
GITHUB_TOKEN_HELP = textwrap.dedent(
    ...
    "`--github-token $(gh auth token)` (https://cli.github.com/) or a PAT "
    "can be generated at https://github.com/settings/personal-access-tokens/new."
)
...
def pytest_addoption(parser: pytest.Parser) -> None:
    ...
    group.addoption(
        ...
        default=None,
        help=(
            "Specify a Github API personal access token (PAT) "
            f"to avoid rate limiting. {GITHUB_TOKEN_HELP}"
        ),
    )
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line(
        "markers",
        "eip_version_check: a test that tests the reference spec "
        "defined in an EIP test module.",
    )
