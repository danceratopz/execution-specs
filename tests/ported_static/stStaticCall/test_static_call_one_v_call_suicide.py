...
Ported from:
tests/static/state_tests/stStaticCall/static_CALL_OneVCallSuicideFiller.json
"""
...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/stStaticCall/static_CALL_OneVCallSuicideFiller.json",  # noqa: E501
    ],
)
...
def test_static_call_one_v_call_suicide(
    ...
    pre.deploy_contract(
        code=(
            Op.SELFDESTRUCT(address=0x8CB4CC1396942231551322A3BA85DA94C3B1EC16)
            + Op.STOP
        ),
        balance=1,
        nonce=0,
    ...
    post = {
        contract: Account(storage={100: 1}),
    }
