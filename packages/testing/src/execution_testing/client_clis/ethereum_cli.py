...
class EthereumCLI:
        ...
        for version_flag, subclasses in groupby(
            ...
            try:
                ...
                if result.returncode != 0:
                    logger.debug(
                        f"Subprocess returncode is not 0! "
                ...
                for subclass in subclasses:
                    ...
                    logger.debug(
                        f"T8n {binary_output} does not match {subclass}"
                    )

            except Exception as e:
                logger.debug(
                    f"Version check with `{version_flag}` failed: {e}"
                )
                continue
    ...
    def detect_binary(cls, binary_output: str) -> bool:
        ...
        logger.debug(
            f"Matching {binary_output} against: {cls.detect_binary_pattern}"
        )
