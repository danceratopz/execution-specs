...
def empty_block_base_fee_preimage(
    *, fork: Fork, base_fee_per_gas: int, gas_limit: int
    ...
    The parent here is genesis, which consumes no gas; a test pinning a
    nonzero ``gas_used`` on its genesis environment would decay from a
    different point than this scan assumes, and its own fee
    expectations would then fail the fill.
    """
...
def empty_block_excess_blob_gas_preimage(
    *, fork: Fork, excess_blob_gas: int, parent_base_fee_per_gas: int
    ...
    Genesis is that parent and uses no blob gas either, so a test
    pinning a nonzero ``blob_gas_used`` on its genesis environment
    would progress from a different point than these candidates assume.

    Some values are unreachable: under a fee floor the decay is skipped
    while the floor holds, so a small nonzero excess has no parent at
    all. Such a test is ineligible for the option and is marked
    ``no_empty_block_fee_preimage`` rather than filled.
    """
