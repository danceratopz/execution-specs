...
def pytest_configure(config: pytest.Config) -> None:
    ...
    if gas_benchmark_values is not None and fixed_opcode_count is not None:
        raise pytest.UsageError(
            f"{GasBenchmarkValues.flag} and --fixed-opcode-count are "
            "mutually exclusive. Use only one at a time."
        )
...
def pytest_collection_modifyitems(
    ...
    # In --fixed-opcode-count mode, we only support tests that meet all
    # of the following:
    ...
    # Note: At this stage we cannot filter based on whether a code
    # generator is used.
