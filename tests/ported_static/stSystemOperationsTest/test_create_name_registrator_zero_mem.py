...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/stSystemOperationsTest/createNameRegistratorZeroMemFiller.json",  # noqa: E501
    ],
)
...
def test_create_name_registrator_zero_mem(
    ...
    contract = pre.deploy_contract(
        code=(
            ...
            + Op.SSTORE(
                key=0x0, value=Op.CREATE(value=0x17, offset=0x3, size=0x0)
            )
            + Op.STOP
        ),
