...
- a chain whose head pins header fields the fork's own parameters admit no block above. Only an intentionally invalid head can: every other header's fields are derived by the fork itself. There are three such fields today - a gas limit below the fork's minimum (which the appended block inherits, and which from Amsterdam on is the budget an empty block's own access list needs), blob gas fields that do not sum within uint64, and (from Osaka on, where [EIP-7918](https://eips.ethereum.org/EIPS/eip-7918) makes a child's excess blob gas depend on its parent's blob gas price) an excess blob gas whose price series would never terminate. No client can derive a child from such a header either - it rejects the head on the same arithmetic - and every one of these bounds is fork arithmetic, so the filler decides them itself instead of asking test authors to re-derive spec constants that change from fork to fork.
