...
class NethtestFixtureConsumer(
    ...
    def consume_state_test_file(
        ...
        try:
            ...
            raise Exception(
                "Failed to parse JSON output on stdout from "
                f"nethtest:\n{result.stdout}"
            ) from e
    ...
    def consume_blockchain_test(
        ...
        if result.returncode != 0:
            raise Exception(
                "nethtest exited with non-zero exit code "
                f"({result.returncode}).\n"
                f"stdout:\n{result.stdout}\n"
                f"stderr:\n{result.stderr}\n"
                f"{' '.join(command)}"
    ...
    def consume_eof_test(
        ...
        if not self.has_eof_support():
            pytest.skip(
                "This version of nethtest does not support the "
                "`--eofTest` flag."
            )
...
class NethermindExceptionMapper(ExceptionMapper):
    ...
    mapping_substring = {
        ...
        BlockException.INCORRECT_BLOB_GAS_USED: (
            "HeaderBlobGasMismatch: Blob gas in header does not match "
            "calculated"
        ),
        ...
        BlockException.INVALID_BLOCK_TIMESTAMP_OLDER_THAN_PARENT: (
            "InvalidTimestamp: Timestamp in header cannot be lower "
            "than ancestor"
        ),
    ...
    mapping_regex = {
        ...
        TransactionException.TYPE_3_TX_MAX_BLOB_GAS_ALLOWANCE_EXCEEDED: (
            r"BlockBlobGasExceeded: A block cannot have more than \d+ "
            r"blob gas, blobs count \d+, blobs gas used: \d+"
        ),
