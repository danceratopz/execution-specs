...
from execution_testing.cli.pytest_commands.plugins.split.grouped_least_duration import (  # noqa: E501
...
def _group_key(item: Item) -> str:
    """
    Return the ``(function_path, fork)`` split-group key for *item*.

    Every parametrization of one test function under one fork maps to
    the same key and lands on the same runner, keeping each
    per-test-function fixture file under its fork subdir
    runner-owned. Items with no ``[`` in their nodeid, or with no
    ``fork_<Name>`` token among the params, form singleton groups
    keyed by the bare nodeid.
    """
    nodeid = strip_xdist_suffix(item.nodeid)
    if "[" not in nodeid:
        return nodeid
    path, _, bracketed = nodeid.partition("[")
...
def pytest_collection_modifyitems(config: Config, items: list[Item]) -> None:
    ...
    keyed_items = [(_group_key(item), item) for item in items]
