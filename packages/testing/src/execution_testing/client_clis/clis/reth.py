...
class RethExceptionMapper(ExceptionMapper):
    ...
    mapping_regex = {
        ...
        TransactionException.INSUFFICIENT_MAX_FEE_PER_BLOB_GAS: (
            r"blob gas price \(\d+\) is greater than max fee per "
            r"blob gas \(\d+\)"
        ),
        ...
        BlockException.INCORRECT_BLOB_GAS_USED: (
            r"blob gas used mismatch|blob gas used \d+ is not a "
            r"multiple of blob gas per blob"
        ),
        BlockException.INCORRECT_EXCESS_BLOB_GAS: (
            r"excess blob gas \d+ is not a multiple of blob gas "
            r"per blob|invalid excess blob gas"
        ),
        BlockException.INVALID_GAS_USED_ABOVE_LIMIT: (
            r"block used gas \(\d+\) is greater than gas limit \(\d+\)"
        ),
        BlockException.INVALID_GASLIMIT: (
            r"child gas_limit \d+ max .* is .*|child gas limit \d+ "
            r"is below the minimum allowed limit"
        ),
        BlockException.INVALID_BLOCK_TIMESTAMP_OLDER_THAN_PARENT: (
            r"block timestamp \d+ is in the past compared to the "
            r"parent timestamp \d+"
        ),
        ...
        BlockException.INVALID_BAL_EXTRA_ACCOUNT: (
            r"Block BAL contains an account change that is not "
            r"present in the computed BAL."
        ),
        BlockException.INVALID_BAL_HASH: (r"Block's access list is invalid."),
        BlockException.INVALID_BAL_MISSING_ACCOUNT: (
            r"Block BAL is missing an account change that is "
            r"present in the computed BAL."
        ),
