...
class FixturesSource:
    ...
    def validate_local_path(path: Path) -> "FixturesSource":
        ...
        if not any(path.glob("**/*.json")):
            pytest.exit(
                f"Specified fixture directory '{path}' contains no JSON files."
            )
...
def pytest_addoption(parser: pytest.Parser) -> None:  # noqa: D103
    ...
    consume_group.addoption(
        ...
        default=None,
        help=(
            "Specify the JSON test fixtures source. Can be a local directory, "
            "a URL pointing to a fixtures.tar.gz archive, a release name and "
            "version in the form of `NAME@v1.2.3` (`stable` and `develop` are "
            "valid release names, and `latest` is a valid version), or the "
            f"special keyword 'stdin'. Defaults to: '{default_input()}'."
        ),
    )
    consume_group.addoption(
        "--cache-folder",
        ...
        help=(
            "Specify the path where downloaded fixtures are cached. "
            f"Defaults to: '{CACHED_DOWNLOADS_DIRECTORY}'."
        ),
    )
    consume_group.addoption(
        "--extract-to",
        ...
        help=(
            ...
            "directly to this path instead of the user's cache directory."
        ),
    )
    ...
    consume_group.addoption(
        ...
        help=(
            "Filter tests by regex pattern or literal test case ID. Prefix "
            "with `id:` to match exact ID (auto-escapes special chars). "
            "Without `id:`, interpreted as Python regex. Prefix with "
            "`collectonly:` to preview matches without executing, e.g. "
            '`--sim.limit "collectonly:.*eip4788.*fork_Prague.*"`. '
            "Set to `collectonly` to list all test case IDs."
        ),
    )
...
def pytest_configure(config: pytest.Config) -> None:  # noqa: D103
    ...
    if config.option.sim_limit:
        if config.option.dest_regex != ".*":
            pytest.exit(
                "Both --sim.limit and --regex flags are set. "
